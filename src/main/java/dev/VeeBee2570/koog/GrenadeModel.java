...
@OnlyIn(Dist.CLIENT)
public class GrenadeModel<E extends Entity> extends EntityModel<E> {
    protected final ModelPart grenade;

    protected GrenadeModel(ModelPart modelPart) {
        this.grenade = modelPart.getChild("grenade");
    }

    public static LayerDefinition createBodyLayer() {
        MeshDefinition meshDefinition = new MeshDefinition();
        PartDefinition partDefinition = meshDefinition.getRoot();
        partDefinition.addOrReplaceChild("grenade", CubeListBuilder.create()
            .texOffs(0,0).addBox(-1,-1,-2,2,2,4)
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int p_103113_, int p_103114_,
        ...
        this.GrenadeParts().forEach((part) -> {
            part.render(poseStack, vertexConsumer, p_103113_, p_103114_);
        });
    }

    protected Iterable<ModelPart> GrenadeParts() {
        return ImmutableList.of(this.grenade);
    }
